...
"""
Original author: Bianka Bakullari
...
def components(event_dic, pairs_list, res_info):
    ...
    AtoR = {a: set([event_dic[ev]['res'] for ev in event_dic.keys() if event_dic[ev]['act'] == a]) for a in A}
    RtoA = {r: set([event_dic[ev]['act'] for ev in event_dic.keys() if event_dic[ev]['res'] == r]) for r in R}
        
    return A, R, S, AtoR, RtoA
